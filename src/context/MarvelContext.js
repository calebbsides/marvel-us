    ...
    characters: [
        {
            ...
            urls: [
                {
                    type: "detail",
                    url: "http://marvel.com/comics/characters/1009351/hulk?utm_campaign=apiRef&utm_source=ebc4ccf5aeca5b699be87a4b2de06286",
                },
                {
                    type: "wiki",
                    url: "http://marvel.com/universe/Hulk_(Bruce_Banner)?utm_campaign=apiRef&utm_source=ebc4ccf5aeca5b699be87a4b2de06286",
                },
                {
                    type: "comiclink",
                    url: "http://marvel.com/comics/characters/1009351/hulk?utm_campaign=apiRef&utm_source=ebc4ccf5aeca5b699be87a4b2de06286",
...
export const MarvelContext = createContext(initialState);

export const MarvelProvider = ({ children }) => {
    const [state, dispatch] = useReducer(MarvelReducer, initialState);

    function lookupCharacters(searchInput) {
        let query = "/api/characters";
        const queryParams = Object.keys(searchInput);
        query += queryParams.length > 0 ? "?" : "";

        queryParams.forEach((key) => {
            query += `${key}=${searchInput[key]}`;
        });
